...
__global__ void softmax_warp_forward(input_t *dst, const output_t *src, int batch_size, int stride, int element_count)
{
    ...
    for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
        ...
        for (int i = 0;i < WARP_BATCH;++i) {
            ...
            val[i] = __shfl_xor(max_value[i], offset, WARP_SIZE);
        }
        #pragma unroll
        for (int i = 0;i < WARP_BATCH;++i) {
    ...
    for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
        ...
        for (int i = 0;i < WARP_BATCH;++i) {
            ...
            sum[i] += __shfl_xor(sum[i], offset, WARP_SIZE);
        }
    }
...
bool dispatch_softmax(output_t *dst, const input_t *src, int softmax_elements, int softmax_elements_stride, int batch_count)
{
    if (softmax_elements == 0) {
        ...
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), dst, src, batch_count, softmax_elements_stride, softmax_elements);
        return true;
    }
    return false;
}
...
bool dispatch_additive_masked_softmax(output_t *dst, const input_t *src, const input_t *pad_mask, int softmax_elements, int softmax_elements_stride, int batch_count, int pad_batch_stride)
{
    if (softmax_elements == 0) {
        ...
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), dst, src, pad_mask, batch_count, softmax_elements_stride, softmax_elements, pad_batch_stride);
        return true;
    }
    return false;
}
...
template<typename input_t, typename output_t, typename acc_t>
bool dispatch_additive_masked_softmax_stream(output_t *dst, const input_t *src, const input_t *pad_mask, int softmax_elements, int softmax_elements_stride, int batch_count, int pad_batch_stride, hipStream_t streamid)
{
    if (softmax_elements == 0) {
        return true;
    } else if (softmax_elements <= 1024) {
        ...
        dim3 threads(warp_size, warps_per_block, 1);
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, streamid, dst, src, pad_mask, batch_count, softmax_elements_stride, softmax_elements, pad_batch_stride);
        return true;
    }
    return false;
}
...
bool dispatch_time_masked_softmax(output_t *dst, const input_t *src, const uint8_t *pad_mask, int softmax_elements, int softmax_elements_stride, int batch_count, int mod_seq_len)
{
    if (softmax_elements == 0) {
        ...
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), dst, src, pad_mask, batch_count, softmax_elements_stride, softmax_elements, mod_seq_len);
        return true;
    }
    return false;
}
...
void dispatch_masked_scale_softmax_backward_masked_out(output_t *grad_input, const input_t *grad, const input_t *output, const uint8_t *mask, const uint8_t *pad_mask, acc_t scale, int softmax_elements, int softmax_elements_stride, int batch_count, int heads)
{
    ...
    if (softmax_elements == 0) {
       return;
        ...
        switch (log2_elements) {
            case 0: // 1
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 0, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 1: // 2
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 1, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 2: // 4
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 2, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 3: // 8
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 3, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 4: // 16
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 4, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 5: // 32
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 5, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 6: // 64
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 6, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 7: // 128
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 7, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 8: // 256
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 8, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 9: // 512
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 9, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 10: // 1024
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 10, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            default:
                break;
...
template<typename input_t, typename output_t, typename acc_t, bool is_log_softmax>
void dispatch_masked_scale_softmax_backward_masked_out_stream(output_t *grad_input, const input_t *grad, const input_t *output, const uint8_t *mask, const uint8_t *pad_mask, acc_t scale, int softmax_elements, int softmax_elements_stride, int batch_count, int heads, hipStream_t streamid)
{
    TORCH_INTERNAL_ASSERT( softmax_elements >= 0 && softmax_elements <= 1024 );
    if (softmax_elements == 0) {
       return;
        ...
        switch (log2_elements) {
            case 0: // 1
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 0, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, streamid, grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
                break;
            case 1: // 2
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t, 1, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, streamid, grad_input, grad, output, mask, pad_mask, scale, batch_count, softmax_elements_stride, softmax_elements, heads);
...
void dispatch_masked_scale_softmax_backward(output_t *grad_input, const input_t *grad, const input_t *output, const uint8_t *mask, acc_t scale, int softmax_elements, int softmax_elements_stride, int batch_count)
{
    ...
    if (softmax_elements == 0) {
       return;
        ...
        switch (log2_elements) {
            case 0: // 1
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 0, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 1: // 2
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 1, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 2: // 4
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 2, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 3: // 8
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 3, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 4: // 16
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 4, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 5: // 32
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 5, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 6: // 64
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 6, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 7: // 128
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 7, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 8: // 256
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 8, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 9: // 512
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 9, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 10: // 1024
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 10, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            default:
                break;
...
template<typename input_t, typename output_t, typename acc_t, bool is_log_softmax>
void dispatch_masked_scale_softmax_backward_stream(output_t *grad_input, const input_t *grad, const input_t *output, const uint8_t *mask, acc_t scale, int softmax_elements, int softmax_elements_stride, int batch_count, hipStream_t streamid)
{
    TORCH_INTERNAL_ASSERT( softmax_elements >= 0 && softmax_elements <= 1024 );
    if (softmax_elements == 0) {
       return;
        ...
        switch (log2_elements) {
            case 0: // 1
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 0, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, streamid, grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 1: // 2
               hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 1, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, streamid, grad_input, grad, output, mask, scale, batch_count, softmax_elements_stride, softmax_elements);
...
void dispatch_softmax_backward_fused_native(output_t *grad_input, const input_t *grad, const input_t *output, int softmax_elements, int softmax_elements_stride, int batch_count)
{
    ...
    if (softmax_elements == 0) {
       return;
        ...
        switch (log2_elements) {
            case 0: // 1
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 0, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 1: // 2
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 1, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 2: // 4
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 2, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 3: // 8
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 3, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 4: // 16
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 4, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 5: // 32
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 5, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 6: // 64
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 6, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 7: // 128
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 7, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 8: // 256
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 8, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 9: // 512
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 9, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            case 10: // 1024
               hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 10, is_log_softmax>)
                    , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
                break;
            default:
                break;
...
bool dispatch_softmax_backward(output_t *grad_input, const input_t *grad, const input_t *output, int softmax_elements, int softmax_elements_stride, int batch_count)
{
    if (softmax_elements == 0) {
        ...
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
        return true;
    }
    return false;
}
...
template<typename input_t, typename output_t, typename acc_t>
bool dispatch_softmax_backward_stream(output_t *grad_input, const input_t *grad, const input_t *output, int softmax_elements, int softmax_elements_stride, int batch_count, hipStream_t streamid)
{
    if (softmax_elements == 0) {
        return true;
    } else if (softmax_elements <= 1024) {
        ...
        dim3 threads(warp_size, warps_per_block, 1);
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, streamid, grad_input, grad, output, batch_count, softmax_elements_stride, softmax_elements);
        return true;
    }
    return false;
}
...
bool dispatch_masked_softmax_backward(output_t *grad_input, const input_t *grad, const input_t *output, const uint8_t *pad_mask, int softmax_elements, int softmax_elements_stride, int batch_count, int pad_batch_stride)
{
    if (softmax_elements == 0) {
        ...
        // launch
       hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), grad_input, grad, output, pad_mask, batch_count, softmax_elements_stride, softmax_elements, pad_batch_stride);
        return true;
    }
    return false;
}
